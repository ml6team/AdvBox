...
class Faster_YOLO:
    ...
    def fc_layer(self, idx, inputs, hiddens, flat=False, linear=False):
        ...
        weight = tf.Variable(tf.random.truncated_normal([dim, hiddens], stddev=0.1))
    ...
    def show_results(self, results):
        ...
        for i in range(len(results)):
            ...
            # draw bbox
            # if self.fromstream:
            out = results[i][0] + ': %.3f' % results[i][5]
            cv2.rectangle(img_cp, (x - w, y - h), (x + w, y + h), (0, 255, 0), 2)
            cv2.rectangle(img_cp, (x - w, y - h - 20), (x + w, y - h), (125, 125, 125), -1)

            # modify showing text
            cv2.putText(img_cp, out,  # + ' : %.2f' % results[i][5]
                        (x - w + 5, y - h - 7),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        3, (0, 0, 0), 1)
                # cv2.putText(img_cp, results[i][5],  # + ' : %.2f' % results[i][5]
                #             (x - w + 5, y - h - 7),
                #             cv2.FONT_HERSHEY_SIMPLEX,
                #             0.5, (0, 0, 0), 1)
    ...
    def run(self):
        if self.fromstream is not None:
            ...
            out = cv2.VideoWriter(outname, fourcc, video_fps,
                                  (int(video_width * imshow_scale), int(video_height * imshow_scale)))
            ...
            resulted_frame = self.detect(self.path_img)
            h_imshow, w_imshow, _ = resulted_frame.shape
            imshow_scale = 1
            drawed_frame = cv2.resize(resulted_frame,
                                      (int(w_imshow * imshow_scale), int(h_imshow * imshow_scale)))
            cv2.imwrite('output.jpg', drawed_frame)
        return None
